...
class _RDFStore:
    ...
    def _try_graphdb(self, nt_path: Path) -> bool:
        ...
        try:
            ...
            if count == 0 and nt_path.exists():
                log.info(f"Repository is empty — uploading {nt_path} …")
                if not self._upload_nt(nt_path):
                    return False
                count = self._graphdb_triple_count()
    ...
    def _sparql_via_http(self, query_string: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return []
    def _update_via_http(self, query_string: str):
    ...
    def execute_sparql(self, query_string: str):
        """
        Execute SPARQL query.
        Supports SELECT/ASK + UPDATE queries.
        Routes to GraphDB (HTTP) or rdflib depending on availability.
        """
        t0 = time.time()
        try:
        ...
        # ✅ READ queries
            if query_upper.startswith("SELECT") or query_upper.startswith("ASK"):
                ...
                log.debug(
                    f"SPARQL SELECT ({'GraphDB' if self._use_graphdb else 'rdflib'}) "
                    f"→ {len(rows)} rows in {elapsed_ms:.1f}ms"
                    )
                return rows
            else:
                if self._use_graphdb:
                    self._update_via_http(query_string)
                else:
                    self._graph.update(query_string)

                elapsed_ms = (time.time() - t0) * 1000
                log.debug(
                    f"SPARQL UPDATE ({'GraphDB' if self._use_graphdb else 'rdflib'}) "
                    f"→ success in {elapsed_ms:.1f}ms"
                    )
